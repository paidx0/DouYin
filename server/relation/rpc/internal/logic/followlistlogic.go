...
func (l *FollowListLogic) FollowList(in *__.ListReq) (*__.ListResp, error) {
	...
	if cnt < 1 {
		return &__.ListResp{StatusCode: 1}, errors.New("用户关注列表空")
	}

	userList := make([]*__.User, 0, cnt)

	// 返回该用户关注列表：包括用户信息，及我是否关注这些用户
	err = global.DBEngine.Table("userfocuson").Where("userfocuson.user_key = ? and userfocuson.deleted_at IS NULL", user.UserKey).
		Join("LEFT", "user", "user.user_key = userfocuson.to_user_key").Where("user.deleted_at IS NULL").
